    ...
    try:
        ...
        for key, value in data.items():
            ...
            if key not in correct_keys:
                return {"error":{"valid_keys": correct_keys,
                    "key_sended": key}}, HTTPStatus.BAD_REQUEST

            if type(value) != str:
                return {"error": f"The value of {key.upper()} only accepted Strings"}, HTTPStatus.BAD_REQUEST
        ...
        if type(e.orig) == UniqueViolation:
            return {"error": f"The title '{work_title}' is alredy exists"}, HTTPStatus.CONFLICT

    except KeyError:
        missing_key = validate_keys.pop()
        
        return {"error": f"key {missing_key.upper()} is missing"}, HTTPStatus.BAD_REQUEST
